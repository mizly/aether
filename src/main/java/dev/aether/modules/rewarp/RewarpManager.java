...
public final class RewarpManager {
    ...
    // Wall-clock backstop per fly attempt so a genuinely hung navigation can't
    // freeze the worker thread. This is only a backstop - the 3s no-progress
    // detector in FlyExecutor is what actually catches stuck flights - so it
    // scales with distance to never guillotine a healthy long flight.
    private static final long REWARP_NAV_TIMEOUT_BASE_MS = 12000L;
    private static final long REWARP_NAV_TIMEOUT_PER_BLOCK_MS = 150L;
    private static final long REWARP_NAV_TIMEOUT_MAX_MS = 90000L;
    ...
    // Final landing must be essentially on the start block, not just "nearby":
    // farming from a couple of blocks off can be the wrong row entirely.
    private static final double REWARP_LANDING_XZ_TOLERANCE = 1.0;
    // Half the player's collision box: the drop probe must cover the whole box,
    // not just the centre line, or a box edge overlapping a wall goes undetected.
    private static final double REWARP_PLAYER_HALF_WIDTH = 0.3;
    ...
    private static double resolveDropLandingY(Minecraft client) {
        Double landingY = queryClientThread(client, () -> {
            ...
            // Probe the whole collision box, not just the centre line: the box
            // edge overlapping a wall block lands on the wall even when the
            // centre column is clear, so we take the highest surface under any
            // corner - that's what the player actually comes to rest on.
            double[][] offsets = {
                    {0, 0},
                    {REWARP_PLAYER_HALF_WIDTH, REWARP_PLAYER_HALF_WIDTH},
                    {REWARP_PLAYER_HALF_WIDTH, -REWARP_PLAYER_HALF_WIDTH},
                    {-REWARP_PLAYER_HALF_WIDTH, REWARP_PLAYER_HALF_WIDTH},
                    {-REWARP_PLAYER_HALF_WIDTH, -REWARP_PLAYER_HALF_WIDTH},
            };
            ...
            for (double[] offset : offsets) {
                Vec3 corner = from.add(offset[0], 0, offset[1]);
                ...
                if (hit.getType() == HitResult.Type.BLOCK) {
                    highest = Math.max(highest, hit.getLocation().y);
                }
            }
    ...
    private static FlyNavResult attemptFlyToStart(Minecraft client, RewarpPointPair pair) {
        for (int attempt = 1; attempt <= REWARP_FLY_MAX_ATTEMPTS; attempt++) {
            ...
            FlyNavResult result = awaitFlyNavigation(client, resolveNavTimeout(client, pair));
            ClientUtils.sendDebugMessage(String.format(
    ...
    private static long resolveNavTimeout(Minecraft client, RewarpPointPair pair) {
        Vec3 pos = getPlayerPosition(client);
        double blocks = 0.0;
        if (pos != null) {
            double dx = pos.x - pair.startX;
            double dy = pos.y - pair.startY;
            double dz = pos.z - pair.startZ;
            blocks = Math.sqrt(dx * dx + dy * dy + dz * dz);
        }
        long timeout = Math.min(REWARP_NAV_TIMEOUT_MAX_MS,
                REWARP_NAV_TIMEOUT_BASE_MS + (long) (blocks * REWARP_NAV_TIMEOUT_PER_BLOCK_MS));
        ClientUtils.sendDebugMessage(String.format(
                "[Rewarp] Nav timeout %.1fs for %.0f blocks.", timeout / 1000.0, blocks));
        return timeout;
    }

    private static FlyNavResult awaitFlyNavigation(Minecraft client, long timeoutMs) {
        ...
        while (PathfindingManager.isNavigating()) {
            ...
            if (System.currentTimeMillis() > deadline) {
                // Backstop only: the 3s no-progress detector should catch real
                // stalls long before this. Hitting it means a genuinely hung nav.
                PathfindingManager.stop(false);
                ClientUtils.sendMessage(String.format(
                        "§cRewarp fly backstop timeout after %.0fs (nav hung).",
                        (System.currentTimeMillis() - startedAt) / 1000.0), false);
                return FlyNavResult.FAILED;
            }
            MacroWorkerThread.sleep(100);
        }
    ...
    private static boolean verifyRewarpLanding(Minecraft client, RewarpPointPair pair) {
        if (!isLandedOnStart(client, pair)) {
            ...
            if (!isLandedOnStart(client, pair)) {
                // Etherwarp refused (server can silently reject it) or unavailable:
                // walk the last stretch. From a wall top this steps off toward the
                // start block and the taps converge on the centre.
                ClientUtils.sendDebugMessage("[Rewarp] Walk-nudging onto the start point.");
                stabilizeFlyPositionForAotv(client, pair);
            }
        }
    ...
    private static boolean isLandedOnStart(Minecraft client, RewarpPointPair pair) {
        ...
        return !Double.isNaN(distance)
                && distance <= REWARP_LANDING_XZ_TOLERANCE
    ...
    private static void ensureFlight(Minecraft client) {
        ...
        // Jump-tapping to start flight can silently fail (e.g. it just made us
        // hop). If we can fly but aren't, force the ability on directly - the
        // same fallback PathfindingManager uses - so the fly never starts grounded.
        if (!isPlayerFlying(client) && canPlayerFly(client)) {
            client.execute(() -> {
            ...
            ClientUtils.sendDebugMessage("[Rewarp] Flight tap failed; force-enabled flying (now "
                    + isPlayerFlying(client) + ").");
        }
    }
